...
def parse_listone(html):
    ...
    for tr in soup.select('tr'):
        ...
        for c in cells[2:]:
            ...
            if c and c.upper() not in ROLES and len(c)<=5: team=c.strip().upper()
...
def parse_fco():
    try:
        ...
        tables=read_html(get_html(FCO)); df=max(tables,key=len); df.columns=[str(c) for c in df.columns]
        def find(terms):
            for c in df.columns:
